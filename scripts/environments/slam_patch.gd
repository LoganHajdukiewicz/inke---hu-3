...
@export var scatter_radius: float = 1.6
...
@export var bump_height: float = 0.45        # How tall the warped bump is
...
func _build_visual():
	# Collision: a low dome-ish cylinder the player can stand/slam on
	var collision = CollisionShape3D.new()
	var shape = CylinderShape3D.new()
	shape.radius = patch_radius
	shape.height = bump_height
	collision.shape = shape
	collision.position.y = bump_height * 0.5
	add_child(collision)
	
	# The warped bump: a squashed hemisphere rising out of the ground
...
func _burst_open(_slammer: Node3D):
	...
	if collectable_scene:
		...
		for i in range(collectable_count):
			...
			var spawn_pos = global_position + Vector3(0, bump_height + 0.3, 0)
			item.global_position = spawn_pos
			
			var angle = (TAU / max(collectable_count, 1)) * i + randf_range(-0.3, 0.3)
			var radius = randf_range(scatter_radius * 0.4, scatter_radius)
			var target = spawn_pos + Vector3(cos(angle) * radius, randf_range(0.3, 0.8), sin(angle) * radius)
	...
	if one_shot:
		# Leave the deflated patch around briefly, then remove collision so
		# the ground is flat again
